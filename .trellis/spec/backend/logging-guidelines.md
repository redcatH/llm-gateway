...
- 上游 5xx 响应体诊断：透明代理对上游 5xx 必须记录响应体才能定位真实错误
  （如讯飞用 HTTP 500 包装的 1105 "system is busy"，不看 body 无法识别真实错误码）。
...
- ❌ 请求体内容（可能含用户对话数据）
- ❌ 上游响应体内容
  - **例外**：上游 5xx 异常诊断时，可记录截断后的响应体，约束如下：
    - 仅 `status >= 500` 触发（4xx 属客户端错误，不记）；
